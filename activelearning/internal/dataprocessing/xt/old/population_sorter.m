function [] = population_sorter(imarisIndex)
%controls:
...
%%%%%%%Bulk population selection mode%%%%%%%%%%%%%%%%%%
%a -- find entire population by setting cuttoff on intensity vector sort
...
% hack to cycle through other possible Imaris Indices
i = 0;
while (isempty(xImarisApp))
        msgbox('Wrong imaris index - trying others');
        imarisIndex = imarisIndex + 1;
        xImarisApp = vImarisLib.GetApplication(imarisIndex);
        if imarisIndex > 5
            msgbox('Exhausted options: close down Imaris, wait 30s and start again');
            return
        end
end
...
%replace any spaces with underscores for maybe name
%maybeName = regexprep(strtrim(maybeName), '\s+', '');
%if (any(strcmp(maybeName, who(surfFile))))
    %selectedImarisIDs = eval(strcat('surfFile.', maybeName)); %read from
    %file
    %add all maybe surfaces in file from previous session
    %func_addsurfacestosurpass(xImarisApp, surfFile, 1, xSurfacesMaybe,
    %selectedImarisIDs);
%else
    %selectedImarisIDs = []; % create empty holder
%end
%xSurfacesMaybe.SetName(selectionName);
...
%         spIdx = find(ismember({stats.Name},'Sphericity'));
    ...
    function [] = initpopulationfindmode()     
        [mostSimilarSurfaceIndices, distances] = recalcintensityvectordistancesort(true);
        %plot histogram of data
        bins = linspace(min(distances),max(distances),100);
        hist(distances,bins)
        xlabel('Hypersphere distance')
        ylabel('Count')
        cutoff = str2double(inputdlg('Histogram cutoff?'));
        %clear existing surfaces of interest to fill in population
        xSurfacesOfInterest.RemoveAllSurfaces;
        func_addsurfacestosurpass(xImarisApp,surfFile,50,xSurfacesOfInterest,stats(1).Ids(mostSimilarSurfaceIndices(distances< cutoff))); 
    end
...
%        drawnow
    ...
    function [] = keyinput(~,~)
        ...
        elseif strcmp(key,'e') % yes the previewed surface is a cell of interest
          ...
          %elseif strcmp(key,'r') % well maybe the previewed surface might be a cell of interest
            %select in file
          %  selectedImarisIDs = unique([selectedImarisIDs; stats(1).Ids(mostSimilarSurfaceIndices(previewIndex))]);
          %  fprintf('total maybe surfaces: %i\n',length(selectedImarisIDs));
            %Select in file (so saves as you go)
          %  eval(strcat('surfFile.',selectionName,' = selectedImarisIDs;'));
            %add surface to imaris (for viewing purposses only)
          %  func_addsurfacestosurpass(xImarisApp,surfFile,1,xSurfacesOfInterest,stats(1).Ids(mostSimilarSurfaceIndices(previewIndex)));               
        ...
        elseif strcmp(key,'a') %find entire population by setting cuttoff on intensity vector sort
            initpopulationfindmode();
...
%         elseif strcmp(key,'7') %mark surface as selected in file
%             selectedIndices = unique([selectedIndices; stats(1).Ids(closestSurfaceIndices(previewIndex))]);
%             fprintf('total selected surfaces: %i\n',length(selectedIndices));
%         elseif strcmp(key,'8') %save selection as sortamato stats
%             fprintf('calculating distances\n');
%             [norms, distancesToMedian, minDistancesToAny] = intensityvectordistancesort(stats,[1 0 1 1 0 0],selectedIndices);         
%             surfFile.stats = stats;
%             fprintf('stats resaved\n');
%         end
    end
