function sortomatocountpartners(~, ~, hSortomatoBase)
    ...
    editGapValue = mycontrol(...
        ...
        'Style', 'edit', ...
        'Tag', 'editGapValue', ...
        'TooltipString', 'Enter a distance allowance for interactions');
    ...
    function pushcount(varargin)
        ...
        countSurfaces = surpassSurfaces(countSurfacesIdx);

        %% Get the partner Surfaces object.
        partnerSurfacesIdx = get(popupPartners, 'Value');
        partnerSurfaces = surpassSurfaces(partnerSurfacesIdx);
        ...
        %% Collect the track data into a struct.
        countStruct(countSurfaceCount) = struct(...
            'Time', [], ...
            'Vertices', [], ...
            'TrackId', []);
        ...
        %% Create a struct for recording all the interaction data.
        countStruct(1:countSurfaceCount) = struct('Partners', 0);

        %% For each track, search for encounters with the target surfaces.
        % For each surface in each track (each time point in the track), we want to
        % keep track of whether it contacts a target surface. After scoring for
        % contacts with the target surface, we determine whether the track meets
        % the criteria to be an interacting track.
        ...
        %% Get the sort Surfaces time and contacts lists.
        countIdxs = 0:countSurfaceCount - 1;
        countTimes = [countStruct(:).Time];
        countContacts = [countStruct(:).Partners] > 0;

        %% Transfer the surface interaction counts to Imaris.
        % Update the status bar and setup the progress bar.
        hStatus.setText('Transferring interactions')
        hStatus.ProgressBar.setValue(0)
        ...
        %% Get the sort tracks lists.
        sortSurfaceParents = [countStruct(:).TrackId];
        sortParentLabels = unique(sortSurfaceParents);
        sortTrackCount = length(sortParentLabels);

        %% Count the total interactions for the tracks.
        sortTrackContactSums = zeros(size(unique(countTrackIds)));
        for s = 1:length(sortParentLabels)
            sortTrackContactSums(s) = sum(countContacts(...
                sortSurfaceParents == sortParentLabels(s)));
        end % for s
        ...
        % Create the unit list.
        contactTrackUnits = repmat({'Contacts'}, [sortTrackCount, 1]); 

        % Assemble the factors cell array.
        contactTrackFactors = cell(3, sortTrackCount);

        % Set the Category to Tracks.
        contactTrackFactors(1, :) = repmat({'Track'}, [sortTrackCount, 1]);

        % Set the Collection to any empty string.
        contactTrackFactors(2, :) = repmat({''}, [sortTrackCount, 1]);

        % Set the Time to an empty string.
        contactTrackFactors(3, :) = repmat({''}, [sortTrackCount, 1]);

        % Send the stats to Imaris.
        countSurfaces.AddStatistics(contactTrackNames, sortTrackContactSums, ...
            contactTrackUnits, contactTrackFactors, contactFactorNames, sortParentLabels')
