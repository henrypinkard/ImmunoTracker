...
try 
    getReferenceVector(dataFile.name);
catch
    storeReferenceVector = 1;
    rawFeatureNames = dataFile.rawFeatureNames;
    rawFeatures = dataFile.rawFeatures;
end
...
%TODO add spot centers based on normalized cut roi if available, or other
%statistic
...
printSelectionInstructions();
    ...
    function [] = printSelectionInstructions()
       ...
       fprintf('3: find intersection of 3 planes \n');
       ...
       fprintf('r: classify and visualize all surfaces (as spots) at all timepoints \n');
...
%Controls
    function [] = keyinput(~,~)
        ...
        elseif strcmp(key,'e') % Classify all    
            ...
            predictAll(1);
        elseif strcmp(key,'r') % Classify all    
            fprintf('Classifying all surfaces...\n');
            predictAll(0);
        ...
        elseif strcmp(key,'y')
            ...
            if storeReferenceVector
                calcAndStoreReferenceVector()
            end
        elseif strcmp(key,'n')
            %Yes the currently presented instance show be laballed as a T cell
            if surfaceClassicationIndex_ ~= 0 && manualPreviewIndex == 0
    ...
    function [] = calcAndStoreReferenceVector()
        if size(dataFile.coiIndices, 1) < 8
            %store reference vector of cell of interest
            intenistyIndices = cell2mat(cellfun(@(toRemove) find(strcmp(rawFeatureNames,toRemove)),...
                {'Intensity Mean - Channel 1', 'Intensity Mean - Channel 2', ...
                'Intensity Mean - Channel 3', 'Intensity Mean - Channel 4', ...
                'Intensity Mean - Channel 5', 'Intensity Mean - Channel 6'}, 'UniformOutput', false));
            vec = mean(rawFeatures(dataFile.coiIndices, intenistyIndices),1);
            normalized = vec ./ sqrt(sum(vec.^2));
            fprintf('Saving reference vector params');
            saveCellTypeParams(dataFile.name, 'referenceVector', normalized);
        end
    end

    function [cls] = retrain(numLearners)
        %train classifier
        cls = trainClassifier([features(coiIndices,:); otherCOIFeatures;
                            features(ncoiIndices,:); otherNCOIFeatures],...
            [ones(length(coiIndices) + size(otherCOIFeatures, 1),1);...
            zeros(length(ncoiIndices) + size(otherNCOIFeatures, 1),1)],numLearners, 0);
    end

    function [] = predictAll(asSurfaces)
        ...
        if asSurfaces
            ...
            addSpotsToSurpass(xImarisApp, imarisIndices(logical(pred)))
        end
    ...
    function [] = predictCurrentTP()
        ...
        currentTPPred = classify( classifier, features, currentTPMask, coiIndices, ncoiIndices, 0);
    ...
    function [] = presentNextExample()
        ...
        [~, currentTPPredValue] = classify( classifier, features, unlabelledAtCurrentTP,coiIndices,ncoiIndices, 0);
